...
if ( owner_id ~= nil ) and ( owner_id ~= NULL_ENTITY ) then
	...
	if ( orbitdist ~= nil ) and ( orbitdir ~= nil ) and ( px ~= nil ) and ( py ~= nil ) then
		...
		if ( dist < 16 ) then
			-- print("Initial vel = " .. math.sqrt( ( vel_y ) ^ 2 + ( vel_x ) ^ 2 ))
		...
		-- print("vel_x = diffx = " .. diffx)
		-- print("vel_y = diffy = " .. diffy)
		
		-- print("vel_total = " .. math.sqrt(( diffx ) ^ 2 + ( diffy ) ^ 2 ))
		-- print()
